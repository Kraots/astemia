...
class Confesscord(commands.Cog):
    ...
    async def confesscord_restrict(self, ctx: Context, *, member: disnake.Member):
        ...
        data: Restrictions = await self.bot.db.get('restrictions', member.id)
        if data is not None:
            return await ctx.reply(
                f'{ctx.denial} {member.mention} is already restricted from using confesscord.'
            )

        await self.bot.db.add('restrictions', Restrictions(
            id=member.id,
    ...
    async def confesscord_unrestrict(self, ctx: Context, *, user: disnake.User):
        ...
        data: Restrictions = await self.bot.db.get('restrictions', user.id)
        if data is None:
        ...
        await self.bot.db.delete('restrictions', {'_id': data.pk})
    ...
    async def confesscord_restrictions(self, ctx: Context):
        ...
        entries = []
        to_append = '{} (`{}`) by {}'
        for data in await self.bot.db.find('restrictions'):
            data: Restrictions
            restricted_by = ctx.astemia.get_member(data.restricted_by)
            mem = ctx.astemia.get_member(data.id)
